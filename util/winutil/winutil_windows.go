...
func LogSupportInfo() {
	...
	log.Printf("Support Info: %s", b.String())
}
...
func getRegistrySupportInfo(root registry.Key, subKeys []string) (map[string]any, error) {
	...
	for _, subKey := range subKeys {
		if err := getRegSubKey(root, subKey, &bufs, output); err != nil && !errors.Is(err, registry.ErrNotExist) {
...
func getRegSubKey(key registry.Key, subKey string, bufs *getRegistrySupportInfoBufs, output map[string]any) error {
...
loopValues:
	...
	if sks, err := k.ReadSubKeyNames(0); err == nil {
		for _, sk := range sks {
			if err := getRegSubKey(k, sk, bufs, kv); err != nil {
				return err
			}
		}
	}
